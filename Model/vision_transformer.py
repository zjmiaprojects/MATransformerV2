...
class dwAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.sr = DW(2*dim, size=31, stride=2)
        self.norm = nn.LayerNorm(2*dim)

    def forward(self, x):
        B, N, C = x.shape
        H = W = int(math.sqrt(N))
        qkv = self.qkv(x).permute(2,0,1)
        # x.reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, kv = qkv[0:C].permute(1,2,0), qkv[C:3*C].permute(1,2,0)  # make torchscript happy (cannot use tensor as tuple)

        kv = kv.permute(0, 2, 1).reshape(B, 2*C, H, W)
        kv = self.sr(kv).reshape(B, 2*C, -1).permute(0, 2, 1)
        kv = self.norm(kv)
...
class Pvt_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,size=4):
        super().__init__()
        ...
        if sr_ratio > 1:
            self.sr = DW(dim,size=size,stride=sr_ratio)
...
class pvtBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,size = 4,sr_ratio=31):
        super().__init__()
        ...
        self.attn = Pvt_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,size=size,sr_ratio=sr_ratio)
...
class DW(nn.Module):
    ...
    def __init__(self, in_channel,size,stride):
        super(DW,self).__init__()

        self.DWConv1 = nn.Sequential(
            nn.Conv2d(in_channel, in_channel, size, stride=stride, padding=(size//2), groups=in_channel),
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patch_embed1 = embed_layer(
            img_size=256, patch_size=8, in_chans=64, embed_dim=embed_dim)
        self.patch_embed2 = embed_layer(
            img_size=128, patch_size=4, in_chans=128, embed_dim=embed_dim)
        self.patch_embed3 = embed_layer(
            img_size=64, patch_size=2, in_chans=256, embed_dim=embed_dim)
        self.patch_embed4 = embed_layer(
            img_size=32, patch_size=1, in_chans=512, embed_dim=embed_dim)
        ...
        sr_ratio = [8,8,8,4,4,4]
        size = [31,31,31,15,15,15]
